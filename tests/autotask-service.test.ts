...
describe("AutotaskService", () => {
  ...
  describe("New Entity Methods", () => {
    ...
    test("should handle expense methods with proper error messages", async () => {
      ...
      await expect(
        service.createExpenseReport({ name: "Test Report", submitterID: 123 }),
      ).rejects.toThrow();

      // Expense items should throw specific error
      await expect(service.getExpenseItem(123, 456)).rejects.toThrow(
        "Expense items API not yet implemented",
      );
      await expect(service.searchExpenseItems(123)).rejects.toThrow(
        "Expense items API not yet implemented",
      );
      await expect(
        service.createExpenseItem(123, {
          description: "Test",
          expenseDate: "2024-01-01",
          expenseAmount: 100,
        }),
      ).rejects.toThrow("Expense items API not yet implemented");
    });
    ...
    test("should handle quote methods with proper error messages", async () => {
      ...
      await expect(
        service.createQuote({ name: "Test Quote", companyID: 123 }),
      ).rejects.toThrow();
    });
    ...
    test("should handle unsupported entity methods with proper error messages", async () => {
      ...
      await expect(service.getBillingCode(123)).rejects.toThrow(
        "Billing codes API not directly available",
      );
      ...
      // Departments
      await expect(service.getDepartment(123)).rejects.toThrow(
        "Departments API not directly available",
      );
      await expect(service.searchDepartments()).rejects.toThrow(
        "Departments API not directly available",
      );
    });
  });
