...
import { MOCK_TICKET_STATUSES, MOCK_TICKET_PRIORITIES, MOCK_ACTIVE_RESOURCES } from '../../setup';

const createHandler = (overrides: Partial<AutotaskService> = {}) => {
  const mockMetadataCache = {
    isValidStatus: (id: number) => MOCK_TICKET_STATUSES.some((s) => s.id === id),
    isValidPriority: (id: number) => MOCK_TICKET_PRIORITIES.some((p) => p.id === id),
    isValidResource: (id: number) => MOCK_ACTIVE_RESOURCES.some((r) => r.id === id),
...
describe('AutotaskToolHandler.autotask_update_ticket', () => {
  test('lists autotask_update_ticket tool with ticketId requirement', async () => {
    const { handler } = createHandler();
    const tools = await handler.listTools();
    const updateTool = tools.find((tool) => tool.name === 'autotask_update_ticket');

    expect(updateTool).toBeDefined();
    const schemaString = JSON.stringify(updateTool?.inputSchema);
    expect(schemaString).toContain('ticketId');
  });
  ...
  test('updates assignment, status, and priority together', async () => {
    ...
    (mockService.updateTicket as jest.Mock).mockResolvedValue(mockUpdatedTicket);
    ...
    const [ticketId, payload] = (mockService.updateTicket as jest.Mock).mock.calls[0];
  ...
  test('fails validation when status is not recognised', async () => {
    ...
    const response = await handler.callTool('autotask_update_ticket', { ticketId: 123, status: 999 });
  ...
  test('fails when no mutable fields are supplied', async () => {
    ...
    const response = await handler.callTool('autotask_update_ticket', { ticketId: 456 });
    ...
    expect(payload.error.guidance).toContain('business logic constraints');
  });
});
