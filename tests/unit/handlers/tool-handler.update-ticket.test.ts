...
describe('AutotaskToolHandler.update_ticket', () => {
  ...
  test('updates assignment, status, and priority together', async () => {
    ...
    const mockUpdatedTicket = { id: 321, status: 2, priority: 3, assignedResourceID: 100 };
    ...
    expect(parsed.data.updatedFields).toEqual(
      expect.arrayContaining(['assignedResourceID', 'status', 'priority', 'queueID', 'description']),
    );
  ...
  test('fails when no mutable fields are supplied', async () => {
    ...
    const payload = JSON.parse(response.content[0].text);
    expect(payload.error.code).toBe('VALIDATION_ERROR');
    expect(payload.error.guidance).toContain('At least one field must be provided');
  });
});
